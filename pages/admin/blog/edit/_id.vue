    ...
    <section class="content">
      <div class="card card-outline card-info">
        <div class="card-header">
          <h3 class="card-title"><i class="nav-icon fas fa-book-open"></i> EDIT BLOG</h3>
        ...
        <div class="card-body">
          <form @submit.prevent="updateBlog">
            ...
            <div class="form-group">
              ...
              <multiselect v-model="blog.category_id" :options="categories" label="name" track-by="id"
...
import Title from '@/components/title.vue';
  export default {
    ...
    mounted() {
      ...
      //fetching data blog
      this.$axios.get(`/api/admin/blogs/${this.$route.params.id}`)

        .then(response => {

          //assing response data to state "blog.title"
          this.blog.title = response.data.data.title

          //assing response data to state "blog.category_id"
          this.blog.category_id = response.data.data.category

          //assing response data to state "blog.content"
          this.blog.content = response.data.data.content

          //assing response data to state "blog.description"
          this.blog.description = response.data.data.description
        })
    ...
    methods: {
      ...
      async updateBlog() {
        ...
        await this.$axios.post(`/api/admin/blogs/${this.$route.params.id}`, formData)
          .then(() => {
